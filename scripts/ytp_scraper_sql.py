...
YTP_KEYWORDS_LIST = [
    ...
    r'YTK', r'WEEGEE',r'YTM',r'You tube poop', r'Youtube\s+poop(?:\s+ITA)?'
]
...
class VideoIndex:
    ...
    def backup_databases(self):
        ...
        for path in [self.ytp_db_path, self.sources_db_path, self.poopers_db_path]:
    ...
    def resort_videos(self):
        print(f"\n  [Resort] Re-sorting all videos between YTP and Sources...")
        ...
        print(f"  [Resort] Scanning {total} videos...", flush=True)
        processed = 0
        ...
        for vid, info in all_videos.items():
            ...
            if is_ytp:
                ...
                if vid in self.sources_data:
                    moved_to_videos += 1
                    clear_line()
                    print(f"    [Move] {vid} -> YTP (Matched: {title[:50]})", flush=True)
            else:
                new_sources_data[vid] = info
                if vid in self.data:
                    moved_to_sources += 1
                    clear_line()
                    print(f"    [Move] {vid} -> Sources (No Match: {title[:50]})", flush=True)
        ...
        clear_line()
        if moved_to_videos == 0 and moved_to_sources == 0:
            print(f"  [Resort] All videos are already in their correct databases.")
        else:
    ...
    def _save_dict_to_sql(self, data_dict, db_type):
        ...
        for vid_id, info in data_dict.items():
            ...
            # 4. Source Pages (sources.db only)
            if db_type == 'sources':
